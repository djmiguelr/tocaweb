...
export function ProgramacionPage() {
  ...
  if (!selectedCity) {
    return (
      <>
        <SEO 
          title="Programación - Toca Stereo"
          description="Descubre nuestra programación diaria. Música, shows en vivo y contenido exclusivo las 24 horas del día."
          type="website"
        />
  ...
  if (isLoading) {
    return (
      <>
        <SEO 
          title={`Programación ${selectedCity?.name ? `- ${selectedCity.name}` : ''} - Toca Stereo`}
          description={`Descubre la programación de Toca Stereo ${selectedCity?.name}. Música, shows en vivo y contenido exclusivo en ${selectedCity?.frequency || 'tu frecuencia local'}.`}
          type="website"
        />
        <div className="min-h-screen pt-20 pb-12 px-4">
  ...
  const dayName = days.find(d => d.id === selectedDay)?.name;

  return (
    <>
      <SEO 
        title={`Programación ${dayName} ${selectedCity?.name ? `- ${selectedCity.name}` : ''} - Toca Stereo`}
        description={`Descubre nuestra programación para ${dayName} en ${selectedCity?.name}. ${currentProgram ? `Ahora al aire: ${currentProgram.name}. ` : ''}Sintonízanos en ${selectedCity?.frequency}.`}
        type="website"
      />
